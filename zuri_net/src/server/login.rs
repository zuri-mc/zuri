...
impl Sequence<Result<(), ConnError>> for LoginSequence {
    async fn execute<'b>(
        ...
        conn.write_packet(&Packet::from(CreativeContent {
            items: vec![],
        }))
            .await;
        conn.flush().await?;
